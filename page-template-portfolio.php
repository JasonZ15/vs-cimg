			...
			<article id="post-<?php the_ID(); ?>" <?php post_class('entry clearfix'); ?>>
				...
				<div class="post-content">
					...
					<div id="et_pt_portfolio_gallery" class="clearfix responsive<?php echo $et_ptemplate_portfolio_class; ?>">
						<h2 style="padding: 0 0 14px 7px;">Keynote Speakers</h2>
						<?php $gallery_query = '';
						$portfolio_count = 1;
						$et_open_row = false;
						if ( !empty($gallery_cats) ) $gallery_query = '&cat=10';
						...
						?>
						<?php query_posts("showposts=$et_ptemplate_gallery_perpage&paged=" . $et_paged . $gallery_query . "&meta_key=last_name&orderby=meta_value&order=ASC"); ?>
						<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
						...
						$portfolio_count = 1;
						$et_open_row = false;
						if ( !empty($gallery_cats) ) $gallery_query = '&cat=8';
						else echo '<!-- gallery category is not selected -->'; ?>
						<?php 
							global $wp_embed;
							$et_videos_output = '';
